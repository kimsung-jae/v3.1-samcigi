...
public final class TripleCore {
    ...
    public static final String PREF = "bubble_triple_hedge_v32";
    public static final String ACTION_UPDATED = "com.bubbleladder.triplehedgev32.TRIPLE_UPDATED";

    public static final String K_HISTORY="history", K_PENDING_IDX="pending_idx_v32", K_PENDING_EXCLUDE="pending_exclude_v32",
            K_PENDING_STAKE="pending_stake_v32", K_PENDING_ODDS="pending_odds_v32", K_PENDING_GRADE="pending_grade_v32",
            K_LIVE_TOTAL="live_total_v32", K_LIVE_SUCCESS="live_success_v32", K_LIVE_PROFIT="live_profit_v32",
            K_BASE_STAKE="base_stake_v32", K_ODDS="odds_v32", K_RECORDS="records_v32", K_AUTO="auto_enabled_v32",
            K_LAST_EXCLUDE="last_exclude_v32", K_LAST_TRIPLE="last_triple_v32", K_LAST_GRADE="last_grade_v32",
            K_LAST_GAP="last_gap_v32", K_LAST_CONTEXT="last_context_v32", K_LAST_SYNC="last_sync_v32";
    ...
    public static final class CompareStats {
        public int v1N,v1Hit,v2N,v2Hit,v3N,v3Hit,v32N,v32Hit;
    ...
    public static final class Analysis {
        ...
        public int exclude,baseExclude,contextId,similarMatches,rescueVotes,v1Exclude;
        public String triple,grade,context,decisionMode;
        public double scoreGap,similarWeight,baseContextRate,v1ContextRate;
        public int baseContextN,v1ContextN;
    ...
    public static List<Result> fetch() throws Exception {
        ...
        c.setRequestProperty("User-Agent","BubbleTripleHedge/3.2");
    ...
    public static Analysis analyze(List<Result> desc){
        ...
        int baseExc=argmin(ens);
        double gap=scoreGap(ens);

        BacktestBundle bt=hedgeTest(asc);
        double rr=bt.stats.rn==0?.75:bt.stats.recentRate();
        String baseGrade=classify(gap,rr,bt.stats.rn);
        RescueVote rv=rescueVote(asc,end);
        int v1Exc=argmin(legacyV1Current(asc,end));
        Decision dec=chooseDecision(ctx,baseGrade,baseExc,rv,v1Exc,bt.policy);
        String finalGrade=calibratedGrade(baseGrade,gap,ctx,dec.strategy,bt.policy);

        CompareStats cmp=compareTest(asc,bt.stats);
        Analysis a=new Analysis();
        a.occurrence=ens;
        a.exclude=dec.exclude;
        a.baseExclude=baseExc;
        a.v1Exclude=v1Exc;
        a.rescueVotes=rv.votes;
        a.decisionMode=dec.mode;
        a.baseContextRate=dec.baseRate;
        a.v1ContextRate=dec.v1Rate;
        a.baseContextN=dec.baseN;
        a.v1ContextN=dec.v1N;
        a.triple=tripleFor(dec.exclude);
        a.grade=finalGrade;
        a.contextId=ctx;
    ...
    private static double adaptiveWeight(Perf global,Perf local,int engine,int ctx){
        ...
        // V3.2: 최근30 안정 엔진은 "안정"이라는 이름만으로 버티지 못한다.
    ...
    private static BacktestBundle hedgeTest(List<Result>a){
        ...
        ArrayDeque<Boolean> hq=new ArrayDeque<>(),bq=new ArrayDeque<>();
        int hr=0,br=0;
        ...
        for(int t=start;t<end;t++){
            ...
            boolean baseOk=actual!=baseExc;
            double rr=hq.isEmpty()?.75:(double)hr/hq.size();
            double gap=scoreGap(ens);
            String baseGrade=classify(gap,rr,hq.size());

            RescueVote rv=rescueVote(a,t);
            int v1Exc=argmin(legacyV1Forecast(a,t,v1n,v1hit,v1q,v1rh));
            Decision dec=chooseDecision(ctx,baseGrade,baseExc,rv,v1Exc,ps);
            String finalGrade=calibratedGrade(baseGrade,gap,ctx,dec.strategy,ps);
            boolean ok=actual!=dec.exclude;

            int gi=gradeIndex(finalGrade),gb=gapBucket(gap);
            h.n++;if(ok)h.hit++;
            h.baseN++;if(baseOk)h.baseHit++;
            ...
            bq.addLast(baseOk);if(baseOk)br++;
            if(bq.size()>50&&bq.removeFirst())br--;

            updatePolicy(ps,ctx,baseGrade,STRAT_BASE,baseOk);
        ...
        h.baseRN=bq.size();h.baseRHit=br;
        BacktestBundle out=new BacktestBundle();out.stats=h;out.policy=ps;return out;
    }
    ...
    private static Decision chooseDecision(int ctx,String baseGrade,int baseExc,RescueVote rv,int v1Exc,PolicyState ps){
        ...
        // V3.2 안전장치: 재투표는 '후보'를 만들 뿐, 과거 동일상황 순차검증이
    ...
    private static CompareStats compareTest(List<Result>a,HedgeStats v32){
    ...
    public static JSONObject backup(Context c) throws Exception {
        ...
        root.put("format","BubbleTripleHedgeV32Backup");
    ...
    public static void restore(Context c,JSONObject root) throws Exception {
        ...
        boolean full="BubbleTripleHedgeV32Backup".equals(format);
        ...
        if(full){
            ...
            // V1/V2/V3.1 import: history + stake/odds only. 실전 성적은 V3.2 A/B 검증을 위해 분리한다.
            if(s.has("base_stake_v3"))ed.putInt(K_BASE_STAKE,Math.max(5000,s.optInt("base_stake_v3",5000)));
            ...
            if(s.has("odds_v3"))ed.putFloat(K_ODDS,(float)s.optDouble("odds_v3",1.95));
