...
public class MainActivity extends Activity {
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리3 · 삼치기 Hedge V3.3 Final",24,Color.WHITE,true));
        TextView sub=tv("V3.1 + V3.2 동시계산 · 같으면 합의 · 다르면 충돌전용 메타검증으로 최종선택",12,Color.rgb(110,231,183),false);
        ...
        hero.addView(tv("다음 회차 V3.3 Final 삼치기",12,Color.GRAY,false));
        ...
        grade=tv("V3.3 신호 -",15,Color.rgb(253,224,71),true);grade.setPadding(0,dp(4),0,0);hero.addView(grade);
        TextView always=tv("● PASS 없음 · V3.1 + V3.2 → Final Meta → 최종 1개 제외",12,Color.rgb(110,231,183),true);always.setPadding(0,dp(10),0,0);hero.addView(always);
        ...
        LinearLayout cc=card();cc.addView(section("V3.1 4확률 경쟁 · V3.2 재판정 · V3.3 최종 심판"));
        candidates=tv("-",15,Color.WHITE,false);candidates.setLineSpacing(0,1.3f);cc.addView(candidates);root.addView(cc);

        LinearLayout ec=card();ec.addView(section("기초 엔진 성적 · V3.1/V3.2 공통 입력"));
        ...
        LinearLayout dc=card();dc.addView(section("데이터 백업 / V1·V2·V3.1·V3.2 기록 가져오기"));
        LinearLayout dr=new LinearLayout(this);dr.setOrientation(LinearLayout.HORIZONTAL);
        backup=button("💾 V3.3 Final 백업",Color.rgb(21,128,61));restore=button("📂 복원/가져오기",Color.rgb(109,40,217));
        ...
        reset=button("V3.3 기록만 초기화",Color.rgb(127,29,29));LinearLayout.LayoutParams rlp=new LinearLayout.LayoutParams(-1,dp(46));rlp.setMargins(0,dp(8),0,0);dc.addView(reset,rlp);root.addView(dc);

        root.addView(tv("균등 무작위라면 삼치기 기준 성공률은 75%입니다. V3.3은 V3.1·V3.2가 충돌한 회차만 따로 학습하며 미래누설 없는 순차검증으로 비교합니다.",11,Color.GRAY,false));
        return sv;
    }
    ...
    private void manualSync(){
        saveSettingsSilent();refresh.setEnabled(false);status.setText("추첨 결과 조회 + V3.1/V3.2/V3.3 Final 분석 중...");
        ex.execute(()->{
            try{
    ...
    private void render(TripleCore.Analysis a,List<TripleCore.Result>d){
        ...
        exclude.setText("V3.3 최종 제외: "+TripleCore.COMBO[a.exclude]+
                " · V3.1: "+TripleCore.COMBO[a.baseExclude]+
                " · V3.2: "+TripleCore.COMBO[a.v32Exclude]);
        grade.setText("V3.3 "+gradeIcon(a.grade)+" "+a.grade+" · "+a.decisionMode+
                " · 격차 "+String.format(Locale.KOREA,"%.1f",a.scoreGap)+" · "+a.context);

        StringBuilder cr=new StringBuilder();
        cr.append("[V3.1 4개 상대점수]\n");
        for(int i=0;i<a.rank.size();i++){
            ...
            if(k==a.baseExclude)cr.append("   · V3.1 제외");
            if(k==a.v32Exclude)cr.append("   · V3.2 제외");
            if(k==a.exclude)cr.append("   ← V3.3 최종");
            if(i<a.rank.size()-1)cr.append("\n");
        }
        cr.append("\n\n[V3.3 Final Meta]")
                .append("\nV3.1 제외: ").append(TripleCore.COMBO[a.baseExclude])
                .append(" · V3.2 제외: ").append(TripleCore.COMBO[a.v32Exclude])
                .append(a.metaAgreement?" · ✅ 같은 값":" · ⚖️ 서로 다른 값")
                .append("\nV3.2 내부판정: ").append(a.v32DecisionMode)
                .append("\nV3.3 최종판정: ").append(a.decisionMode)
                .append("\n메타보정점수 V3.1 ").append(TripleCore.pct(a.metaV31Score))
                .append(" · V3.2 ").append(TripleCore.pct(a.metaV32Score))
                .append(" · 근거표본 ").append(a.metaEvidenceN).append("회")
                .append("\n유사상황 ").append(a.similarMatches).append("건 · 기초모델 반영 ")
                .append(String.format(Locale.KOREA,"%.0f%%",a.similarWeight*100)).append(" (최대 18%)")
                .append("\n5엔진 최다합의: ").append(a.rescueVotes).append("/5")
                .append("\n동일상황 V3.1 보정성적 ").append(a.baseContextN).append("회 ").append(TripleCore.pct(a.baseContextRate))
                .append(" · V1 ").append(a.v1ContextN).append("회 ").append(TripleCore.pct(a.v1ContextRate));
        candidates.setText(cr);

        StringBuilder es=new StringBuilder();
        es.append("현재 상황: ").append(a.context).append(" · 표본보호 유지\n")
                .append("V3.1은 4확률 경쟁 · V3.2는 약/연속 구조요청 · V3.3은 둘의 충돌만 메타학습\n")
                .append("최근30 안정 엔진은 최근성적 75% 미만이면 자동 감점\n\n");
        ...
        TripleCore.HedgeStats h32=a.hedge32;
        TripleCore.CompareStats cp=a.compare;
        TripleCore.MetaStats mt=a.meta;
        backtest.setText(
                "V3.3 전체: "+ht.hit+"/"+ht.n+(ht.n>0?" = "+TripleCore.pct(ht.rate()):"")+
                "\nV3.3 최근 50회: "+ht.rhit+"/"+ht.rn+(ht.rn>0?" = "+TripleCore.pct(ht.recentRate()):"")+
                "\nV3.2 최근 50회: "+h32.rhit+"/"+h32.rn+(h32.rn>0?" = "+TripleCore.pct(h32.recentRate()):"")+
                "\nV3.1 최근 50회: "+h32.baseRHit+"/"+h32.baseRN+(h32.baseRN>0?" = "+TripleCore.pct((double)h32.baseRHit/h32.baseRN):"")+
                "\n\n[V1 · V2 · V3.1 · V3.2 · V3.3 같은 구간]"+
                "\nV1 방식: "+cp.v1Hit+"/"+cp.v1N+" = "+TripleCore.pct(cp.v1Rate())+
                "\nV2 방식: "+cp.v2Hit+"/"+cp.v2N+" = "+TripleCore.pct(cp.v2Rate())+
                "\nV3.1 방식: "+cp.v3Hit+"/"+cp.v3N+" = "+TripleCore.pct(cp.v3Rate())+
                "\nV3.2 방식: "+cp.v32Hit+"/"+cp.v32N+" = "+TripleCore.pct(cp.v32Rate())+
                "\nV3.3 방식: "+cp.v33Hit+"/"+cp.v33N+" = "+TripleCore.pct(cp.v33Rate())+
                "\n\n[V3.1 ↔ V3.2 합의/충돌 전용검증]"+
                "\n같은 값: "+bucketStat(mt.agreeHit,mt.agreeN)+
                "\n서로 다른 값: "+mt.conflictN+"회"+
                "\n  V3.1 포함성공: "+bucketStat(mt.conflictV31Hit,mt.conflictN)+
                "\n  V3.2 포함성공: "+bucketStat(mt.conflictV32Hit,mt.conflictN)+
                "\n  V3.3 최종성공: "+bucketStat(mt.conflictFinalHit,mt.conflictN)+
                "\n  V3.3→V3.1 선택: "+bucketStat(mt.choose31Hit,mt.choose31N)+
                "\n  V3.3→V3.2 선택: "+bucketStat(mt.choose32Hit,mt.choose32N)+
                "\n  최근 충돌 "+mt.recentN+"회: V3.1 "+bucketStat(mt.recent31Hit,mt.recentN)+
                " · V3.2 "+bucketStat(mt.recent32Hit,mt.recentN)+
                "\n\n[상황별 V3.3 성공률]"+
        ...
        live.setText("V3.3 실전 삼치기 · "+(n>0?hit+"/"+n+" = "+TripleCore.pct((double)hit/n)+" · 누적 "+TripleCore.signed(lp):"아직 없음"));
        ...
        profit.setText(
                ...
                "\nV3.3 순차검증 가상손익: "+TripleCore.signed(bt)
        );
    ...
    private String metaContext(TripleCore.MetaStats m,int i){
        int n=m.contextN[i];
        return n==0?"-":n+"회 · V3.1 "+TripleCore.pct((double)m.context31Hit[i]/n)+" / V3.2 "+TripleCore.pct((double)m.context32Hit[i]/n);
    }
    ...
    private void updateBgState(){
        ...
        bgState.setText(on?"● 백그라운드 ON · 추첨 후 자동조회/채점/V3.3 Final 재계산":"○ 백그라운드 OFF");
    ...
    private void startExport(){
        ...
        i.putExtra(Intent.EXTRA_TITLE,"BubbleTripleHedgeV33_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");
    ...
    private void exportUri(Uri u){
        try{
            ...
            Toast.makeText(this,"V3.3 Final 백업 완료",Toast.LENGTH_LONG).show();
    ...
    @Override protected void onActivityResult(int rc,int res,Intent data){
        ...
        if(rc==REQ_EXPORT)exportUri(u);
                ...
                .setMessage("V3.3 백업은 전체 복원하고, V1/V2/V3.1/V3.2 백업은 결과 기록과 기본설정만 가져옵니다.")
                .setNegativeButton("취소",null).setPositiveButton("가져오기",(d,w)->importUri(u)).show();
    }

    private void confirmReset(){
        new AlertDialog.Builder(this).setTitle("V3.3 기록 초기화")
                .setMessage("V3.3의 누적 결과·실전 적중·수익 기록을 삭제합니다. V1/V2/V3.1/V3.2 앱에는 영향이 없습니다.")
